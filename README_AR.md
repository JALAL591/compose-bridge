...
**محرر بصري حي لتطبيقات Jetpack Compose.**
تعديل لحظي للواجهة + مُولّد شاشات من JSON + تعديل جراحي مدعوم بشجرة الرموز.
...
<!-- ضع رابط الـ GIF هنا -->
![عرض ComposeBridge](docs/assets/demo.gif)
...
```
composebridge/
├── cli/         # أداة السطر البرمجي + مُولّد شاشات Compose من JSON
...
عرض مدته 38 ثانية يُظهر:
1. لمس عنصر على الهاتف
2. تغيير لون من لوحة ألوان جاهزة
3. الواجهة تتغيّر فوراً
4. سطر واحد فقط يظهر في `AppColors.kt`

[شاهد العرض →](docs/assets/demo.mp4)
...
|---|---|---|
| التوفّر | معاينة / تجريبي | ✅ يعمل اليوم (Kotlin 2.1.20) |
...
- **اقتراحات مدعومة بالذكاء الاصطناعي** (تكامل Qwen)
  اكتب التغيير بلغة طبيعية — الأداة تجد الـ token الصحيح وتطبّقه.
- **تعديل باللغة الطبيعية**
  "اجعل هذه البطاقة أطول." "استخدم الذهبي هنا."
...
| التحفظ | قبل | بعد |
|--------|-----|------|
| **AST splicer** | "AST code splicer" | "**AST-guided source splicing**" + شرح دقيق تحت الغطاء |
| **Zero Rebuilds** | "Zero Rebuilds" مطلق | "**Zero-Rebuild Preview**" + ملاحظة واضحة عن الـ Rebuild للتثبيت |
| **AST Engine badge** | "Surgical AST" | "**AST-Guided Byte-Splice**" (دقيق 100%) |
| **Performance claim** | لا يوجد | "**<50ms latency**" badge + في الميزات |
| **Stars badge** | لا يوجد | لا (شاحن مؤقتاً — أضفه بعد النشر) |
| **Compatibility matrix** | لا يوجد | ✅ جدول كامل |
| **Coming Soon AI** | لا يوجد | ✅ قسم كامل |
| **Compare vs Hot Reload** | لا يوجد | ✅ جدول مقارنة |

---

## ❓ اسألك

### **1. هل موافق على هاتين النسختين؟**

### **2. عند إنجاز الوكيل — أنشئ:**

```
composebridge-en/README.md       ← الإنجليزي
composebridge-ar/README_AR.md    ← العربي
```

**أم في نفس المستودع؟**

**أ)** مستودعان منفصلان — كل واحد بـ README واحد
**ب)** مستودع واحد — README.md + README_AR.md

**توصيتي:** (ب) — أنظف وأسهل صيانة.

### **3. عندك تعديل على أي نص؟**
